...
def ask_rag_session(
    ...
    if not session:
        session = ChatSession(user_id=current_user.id, title=query.question[:30] + ("..." if len(query.question) > 30 else ""))
        db.add(session)
        db.commit()
        db.refresh(session)
...
def ask_with_attachment(
    ...
    session = db.query(ChatSession).filter(ChatSession.id == session_id).first()
    if not session:
        session = ChatSession(user_id=current_user.id, title=question[:30] + ("..." if len(question) > 30 else ""))
        db.add(session)
        db.commit()
        db.refresh(session)
